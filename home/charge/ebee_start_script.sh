...
function_cleanup_fs()
{
    # delete any possible leftovers from opkg extractions/installations
    rm -rf /home/charge/opkg-*
}
...
function_fix_master_slave()
{
	...
	# Implement correct logic and check for ON-settings first!
	...
	elif grep -q '^Off' /home/charge/persistency/MasterMode_ms 2>/dev/null
	then
        # necessary for Mennekes, which has different defaults:
		echo "Off" > /home/charge/persistency/MasterSlaveMode_ms
	elif grep -q '^Off' /home/charge/persistency/SlaveMode_ms 2>/dev/null
	then
        # necessary for Mennekes, which has different defaults:
		echo "Off" > /home/charge/persistency/MasterSlaveMode_ms
	fi
